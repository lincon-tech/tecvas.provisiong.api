...
using Chams.Vtumanager.Provisioning.Services.BillPayments.Spectranet;
...
namespace Chams.Vtumanager.Provisioning.Api.Controllers.v1
{
    ...
    public class SpectranetController : Controller
    {
        private readonly ILogger<SpectranetController> _logger;
        
        private readonly ISpectranetPaymentsService _billspaymentService;
        private readonly ITransactionRecordService _transactionRecordService;


        
        public SpectranetController(
            ILogger<SpectranetController> logger,
            ISpectranetPaymentsService billspaymentService
            )
        {
            _logger = logger;
        ...
        public async Task<IActionResult> Refill(
            SpectranetRefillRequest renewRequest,
            CancellationToken cancellation)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var dstvresponse  = await _billspaymentService.SpectranetRefillAsync(renewRequest, cancellation);
        ...
        public async Task<IActionResult> PaymentPlan(
           SpectranetPaymentPlanRequest renewRequest,
           CancellationToken cancellation)
        {
            await Task.Delay(0, cancellation).ConfigureAwait(false);
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var dstvresponse = await _billspaymentService.SpectranetPaymentPlanAsync(renewRequest, cancellation);
        ...
        public async Task<IActionResult> Pin(
           SpectranetPINRequest renewRequest,
           CancellationToken cancellation)
        {
            await Task.Delay(0, cancellation).ConfigureAwait(false);
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var dstvresponse = await _billspaymentService.SpectranetPINAsync(renewRequest, cancellation);
