...
using Chams.Vtumanager.Provisioning.Services.BillPayments.IkejaELectric;
...
namespace Chams.Vtumanager.Provisioning.Api.Controllers.v1
{
    ...
    public class IkejaElectricController : Controller
    {
        private readonly ILogger<IkejaElectricController> _logger;
        
        private readonly IIkejaElectricPaymentsService _billspaymentService;
        private readonly ITransactionRecordService _transactionRecordService;


        
        public IkejaElectricController(
            ILogger<IkejaElectricController> logger,
            IIkejaElectricPaymentsService billspaymentService
            )
        {
            _logger = logger;
        ...
        public async Task<IActionResult> BillPayments(
            IkejaElectricRequest renewRequest,
            CancellationToken cancellation)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var dstvresponse  = await _billspaymentService.IkejaElectricPrepaidAsync(renewRequest, cancellation);
