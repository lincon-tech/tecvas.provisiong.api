...
using Chams.Vtumanager.Provisioning.Services.BillPayments.Startimes;
...
namespace Chams.Vtumanager.Provisioning.Api.Controllers.v1
{
    ...
    {
        private readonly ILogger<StartimesController> _logger;
        
        private readonly IStartimesPaymentsService _billspaymentService;
        private readonly ITransactionRecordService _transactionRecordService;


        
        public StartimesController(
            ILogger<StartimesController> logger,
            IStartimesPaymentsService billspaymentService
            )
        {
            _logger = logger;
            ...
            CancellationToken cancellation)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var dstvresponse  = await _billspaymentService.StartimesPaymentAsync(renewRequest, cancellation);
