...
using Chams.Vtumanager.Provisioning.Services.BillPayments.EkoElectric;
...
namespace Chams.Vtumanager.Provisioning.Api.Controllers.v1
{
    ...
    public class EkoElectricController : Controller
    {
        private readonly ILogger<EkoElectricController> _logger;
        
        private readonly IEkoElectricPaymentsService _billspaymentService;
        private readonly ITransactionRecordService _transactionRecordService;


        
        public EkoElectricController(
            ILogger<EkoElectricController> logger,
            IEkoElectricPaymentsService billspaymentService
            )
        {
            _logger = logger;
        ...
        public async Task<IActionResult> PrepaidPayments(
            EkoElectricPostpaidRequest renewRequest,
            CancellationToken cancellation)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var dstvresponse  = await _billspaymentService.EkoElectricPostpaidAsync(renewRequest, cancellation);
            ...
            {
                _logger.LogError($"Api failure in PrepaidPayments with error message {ex.Message}  error details {ex}");
        ...
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> PrepaidPayments(
            EkoElectricPrepaidRequest renewRequest,
            CancellationToken cancellation)
        {
            await Task.Delay(0, cancellation).ConfigureAwait(false);
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var dstvresponse = await _billspaymentService.EkoElectricPrepaidAsync(renewRequest, cancellation);
